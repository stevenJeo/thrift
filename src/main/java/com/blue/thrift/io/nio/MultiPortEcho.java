...
public class MultiPortEcho {
        ...
        private void go() throws IOException {
            ...
            for (int i = 0; i < ports.length; ++i) {
                ...
                ssc.socket().bind(new InetSocketAddress(ports[i]));//监听一个端口
                //3. 注册到selector
            ...
            while (true) {
                ...
                while (it.hasNext()) {
                    ...
                    if ((key.readyOps() & SelectionKey.OP_ACCEPT) == SelectionKey.OP_ACCEPT) {
                        ...
                        SocketChannel sc = (SocketChannel) key.channel();// Read the data

//                        int bytesEchoed = 0;// Echo data
//                        while (true) {
//                            echoBuffer.clear();
...
//                        }
                        ...
                        sc.read(echoBuffer);
                        echoBuffer.flip();
                        byte[] buffer = new byte[echoBuffer.limit()];
                        echoBuffer.get(buffer);

                        System.out.println("Echoed " + new String(buffer) + " from " + sc);
                        echoBuffer.flip();
                    }
                    it.remove();
                }
